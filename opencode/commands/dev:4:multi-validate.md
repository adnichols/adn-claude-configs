...
4. **Add Validation Findings as HTML Comments**
   ...
   <!-- [GLM Reviewer] INCORRECT - Task 2.3: Spec requires authentication headers (Bearer token) but implementation uses cookies. -->
   <!-- [GLM Reviewer] SCOPE DRIFT - Phase 3 added admin panel that is not in specification or task list. -->
   <!-- [GLM Reviewer] MISINTERPRETATION - Task 1.5 spec says "soft delete" but implementation does hard delete. -->
   <!-- [GLM Reviewer] CONTRADICTION - Task 4.2 implementation conflicts with specification section 5.3 which states "read-only access". -->
   <!-- [GLM Reviewer] WRONG REFERENCE - Task 2.1 references src/api/users.ts but actual path is src/services/user.service.ts. -->
   
   For passing validation (optional, provides confidence):
   <!-- [GLM Reviewer] PASS - Task 1.0: Implementation correctly matches specification requirements. -->
...
6. **Critical Flagging:**
   ...
   <!-- [GLM Reviewer] CRITICAL - Task 3.1: No authentication on API endpoint. Any user can delete data. -->
...
4. **Add Validation Findings as HTML Comments**
   Format: <!-- [Kimi Reviewer] [CATEGORY] - Task X.Y: Your finding. -->
...
**Reviewer Findings:**
[Extract HTML comments from task list for this task]
