...
Spin up a ready-to-code environment for a Linear issue. Accept the Linear issue key as the first argument (for example `NOD-123`). Optionally accept a second argument that overrides the default base branch (`origin/main`). 

Linear issue and optional base branch: $ARGUMENTS 

Also follow this repository's `AGENTS.md` for project-specific branching, environment, and tooling conventions.
...
Use the `linear` CLI to fetch issue details and verify project membership:

1. Fetch structured issue JSON:
   ```bash
   linear issues --filter "id:<ISSUE_KEY>" --format json --limit 1
   ```
2. Parse JSON to extract: `title`, `description`, `project.name`, `state.name`, `labels`, `url`.
3. Verify `project.name` equals `"Doc Thingy"` (case-sensitive). If it does not, halt with an error explaining the mismatch.
4. Ensure `linear whoami` succeeds; if it fails, instruct the user to run `linear auth` before proceeding.
