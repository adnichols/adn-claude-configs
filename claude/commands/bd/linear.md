---
description: Manage Linear tickets with beads tracking
---

# Linear - Ticket Management (with Beads Integration)
...
First, verify that the `ltui` CLI tool is available by running `ltui --version`. If not available, respond:
```
I need access to the ltui CLI tool to help with ticket management. Please ensure ltui is installed and configured, then try again.
```

If ltui is available, respond based on the user's request:
...
When referencing thoughts documents, always provide GitHub links using the `links` parameter if the repository is on GitHub.

### Default Values
- **Status**: Always create new tickets in "Triage" status (or ask user for their preferred default)
- **Project**: Ask user which project to use, or check .ltui.json for project defaults
...
Automatically apply labels based on the ticket content and project conventions. Use `ltui labels list` to see available labels for the team.
...
1. **Locate and read the thoughts document:**
   ...
   - Create beads issues to track the process:
     ```bash
     bd create "Read and analyze thoughts document" --type task
     bd create "Draft Linear ticket" --type task
     bd create "Get user approval and create ticket" --type task
     bd update <first-id> --status in_progress
     ```
...
3. **Check for related context (if mentioned in doc):**
   ...
   - Close the first beads issue:
     ```bash
     bd close <first-id> --reason "Analysis complete"
     bd update <second-id> --status in_progress
     ```

4. **Get Linear workspace context:**
   - List teams: `ltui teams list`
   - If multiple teams, ask user to select one
   - List projects for selected team: `ltui projects list`
...
8. **Post-creation actions:**
   - Close the beads issues:
     ```bash
     bd close <second-id> --reason "Ticket drafted"
     bd close <third-id> --reason "Ticket created"
     bd sync
     ```
...
1. **Determine which ticket:**
   ...
   - If uncertain, use `ltui issues view` to show ticket details and confirm with user
...
4. **Comment structure example:**
   ...
   Files updated:
   - `backend/webhooks/handler.go` ([GitHub](link))
   - `thoughts/rate_limit_analysis.md` ([GitHub](link))
   ```
...
Use ltui commands to discover IDs dynamically:
- Teams: `ltui teams list`
- Projects: `ltui projects list --team <team>`
- Labels: `ltui labels list --team <team>`
- Workflow states: `ltui states list --team <team>`
- Users: `ltui users list --team <team>`
