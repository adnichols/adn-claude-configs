...
# Usage: bash /path/to/adn-claude-configs/scripts/update.sh [target-directory]
...
if [ "$CLAUDE_INSTALLED" = false ] && [ "$CODEX_INSTALLED" = false ]; then
    ...
    echo "  bash $SCRIPT_DIR/install.sh --all"
    exit 1
fi
...
update_codex() {
    ...
    # Skip CODEX.md
    echo -e "  ${YELLOW}✓ Skipped CODEX.md (generated by codex)${NC}"
